...
=======
    var q = quotes.replace(/-/g,'%20')
>>>>>>> b78f3a6e1b7c517cfd6911e1d055643c6c115e7a
...
const weather = async (kota) => {
    const response = await fetch(`http://api.openweathermap.org/data/2.5/weather?q=${kota}&appid=4355651c60d51ccbc987c67ee4498ad1`);
    if (!response.ok) throw new Error(`unexpected response ${response.statusText}`);
    const json = await response.json()
